...
export default function ReportsPage() {
  ...
  const fetchReports = async (start?: string, end?: string, pageNumber = 1) => {
    ...
    let baseQuery = supabase
      ...
      `, { count: 'exact' }) // penting untuk mendapatkan total count
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleFilter} className="mb-6 flex gap-4 items-end flex-wrap">
        <div>
          <label className="block mb-1 font-semibold" htmlFor="startDate">Start Date</label>
          <input
            id="startDate"
            type="date"
            value={startDate}
        ...
        <div>
          <label className="block mb-1 font-semibold" htmlFor="endDate">End Date</label>
          <input
            id="endDate"
            type="date"
            value={endDate}
        ...
        <button type="submit" className="bg-blue-600 text-white px-5 py-2 rounded hover:bg-blue-700 transition">Filter</button>
        <button
          type="button"
          onClick={() => {
            setStartDate('');
      ...
      <div className="space-y-6">
        {transactions.map((trx) => (
          <div key={trx.id} className="border p-4 rounded shadow">
            <div className="mb-2 text-gray-700"><strong>ID:</strong> {trx.id}</div>
            <div className="mb-2 text-gray-700"><strong>Tanggal:</strong> {dayjs(trx.created_at).format('DD MMM YYYY HH:mm')}</div>
            <div className="mb-2">
              <strong>Items:</strong>
            ...
            <div className="mt-2 font-semibold">Total: Rp{trx.total.toLocaleString()}</div>
          </div>
        ))}
      </div>
